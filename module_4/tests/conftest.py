...
def test_db():
    # 1. Connect to 'postgres' maintenance database to manage 'gradcafe_test'
    default_dsn = get_db_dsn(env_overrides={"PGDATABASE": "postgres"})
    conn = psycopg2.connect(default_dsn)
    conn.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
    cur = conn.cursor()
    
    # 2. Terminate all other connections to avoid ObjectInUse errors
    cur.execute("""
        SELECT pg_terminate_backend(pid) 
        FROM pg_stat_activity 
        WHERE datname = 'gradcafe_test' AND pid <> pg_backend_pid();
    """)
    
    # 3. Safely recreate the test database
